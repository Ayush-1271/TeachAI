...
interface AttendanceContextType {
  ...
  sessions: Session[];
  createSession: (
    session: Omit<Session, "id" | "teacherId">
  ) => Promise<Session>;
  markAttendance: (
    sessionCode: string,
    faceMatched: boolean,
...
export const AttendanceProvider: React.FC<{ children: React.ReactNode }> = ({
  ...
  const createSession = async (
    sessionData: Omit<Session, "id" | "teacherId">
  ): Promise<Session> => {
  ...
  const getPreviousSessions = (): Session[] => {
    ...
    return sessions.filter((session) => {
      ...
      // A session is considered previous if it is not active
      return !isActive;
    });
  };
  ...
  const markAttendance = async (
    ...
    if (user?.role !== "student" || !user.studentId) {
      return {
        success: false,
        message: "You must be logged in as a student to mark attendance",
      };
    }
  ...
  const manuallyUpdateAttendance = (
    recordId: string,
    status: "present" | "absent"
  ) => {
    if (user?.role !== "teacher") {
